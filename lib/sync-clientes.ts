...
const atributos = [
  ...
  'Observacion', 'CondicionAnteElIVA', 'IngresosBrutos',
  'ContactoDeVenta', 'ContactoDeCobros', 'CondicionPago',
  'MonedaUsualCuentaCorriente', 'CuentaCliente', 'TipoDeCliente',
  'ActividadDeCliente', 'Vendedor', 'ZonaDeVenta',
  'Cobrador', 'BloqueadoParaNotasDePedido',
  'BloqueadoParaFacturar', 'FechaDeAlta', 'FechaDeBaja',
  'HabilitadoParaConsultasWeb', 'FechaUltActualizacion'
  // Eliminamos Clasificacion1-9, ClaveTributaria, Transporte, FormatoDeImpresion, Vendedor2
];
...
export async function syncClientes() {
  ...
  await syncGenerico({
    ...
    tabla: 'clientes',
    idCol: 'clienteid',
    limite: 100, // Para pruebas rápidas
    mapear: (item: any) => {
